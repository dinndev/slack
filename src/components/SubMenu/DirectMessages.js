...
const DirectMessages = ({myFunction}) => {
    ...
    const [doneFetchingOfChannelMembers, setDoneFetchingOfChannelMembers] = useState(false);
            ...
            uniqMembers.map((theUser)=>{
                ...
                .then(response => {
                    if(response.data.data.length !== 0 ){
                        setMessengers([...messengers, {id: theUser, email: response.data.data[0].receiver.uid == user.uid ? response.data.data[0].sender.uid : response.data.data[0].receiver.uid}])
                    }
                })
    ...
    useEffect(()=>{
        let uniqueListUID = messengerList.filter((value, index, self) =>
